...
bin_cmds = node['wljava']['bin_cmds']
default = node['wljava']['set_default']
priority = node['wljava']['alternatives_priority']
owner = node['wljava']['owner']
group = node['wljava']['group']
java_home = node['wljava']['java_home']
...
ruby_block 'set JAVA_HOME in /etc/environment' do
  ...
  only_if { node['wljava']['set_etc_environment'] }
end
...
if default && node['wljava']['use_alt_suffix']
  ...
  block do
    ...
    bin_cmds.each do |cmd|
      ...
      unless alternative_exists
        ...
        if node['wljava']['reset_alternatives']
