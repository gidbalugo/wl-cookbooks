...
win_jre_home = node['wljava']['win_jre_home']
...
execute "install #{::File.basename(file_cache_path)} to #{java_home} with JRE #{win_jre_home}" do
  command "\"#{file_cache_path}\" /s ADDLOCAL=\"#{node['wljava']['win_addlocal']}\" #{install_dir} #{jre_install_dir}"
  not_if { ::File.exist?(java_home) }
end

win_javalink = node['wljava']['win_javalink']
