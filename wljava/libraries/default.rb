def java_arch
  ...
  arch = 'i586' if node['wljava']['force_i586'] && !platform?('mac_os_x')
  arch
end

def java_version
  "1.#{node['wljava']['release']}.0_#{node['wljava']['update']}"
end

def jdk_version
  "#{node['wljava']['release']}u#{node['wljava']['update']}"
end
...
def fetch_java_installer
  ...
  when 'mac_os_x'
    checksum = node['wljava']['sha256']['dmg']['x64']
    jdk = "jdk-#{jdk_version}-macosx-#{java_arch}.dmg"
  when 'windows'
    checksum = node['wljava']['sha256']['exe'][java_arch]
    jdk = "jdk-#{jdk_version}-windows-#{java_arch}.exe"
  else
    checksum = node['wljava']['sha256']['tar'][java_arch]
    jdk = "jdk-#{jdk_version}-linux-#{java_arch}.tar.gz"
  end

  uri = node['wljava']['uri']
  if uri.nil? || uri.empty?
    download_url = "http://download.oracle.com/otn-pub/java/jdk/#{jdk_version}-b#{node['wljava']['build']}/#{jdk}"
